...
void MAPSros2_topic_float32_array::Dynamic()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        m_outputName = Name();
        m_outputName += "_" + m_inOutName;
        NewOutput("output_float32_array", m_outputName.c_str());
...
bool MAPSros2_topic_float32_array::Birth()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        ...
        std::function<void(const std_msgs::msg::Float32MultiArray::SharedPtr msg)> fnc =
        std::bind(&MAPSros2_topic_float32_array::TopicCallback, this, std::placeholders::_1);
        m_sub = m_node->create_subscription<std_msgs::msg::Float32MultiArray>(m_topicName, 10, fnc);
    }
    else
    {
        m_pub = m_node->create_publisher<std_msgs::msg::Float32MultiArray>(m_topicName, 100);
    }

    return true;
}

void MAPSros2_topic_float32_array::Core()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
        return;
    ...
    {
        message.data[i] = ioeltin->Float32(i);
    }
    ...
    message.layout.dim[0].stride=vectorsize_in*sizeof(MAPSFloat32);

    m_pub->publish(message);
}

void MAPSros2_topic_float32_array::Death()
{
    m_pub.reset();
    m_sub.reset();
}

void MAPSros2_topic_float32_array::TopicCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
    try 
    {
