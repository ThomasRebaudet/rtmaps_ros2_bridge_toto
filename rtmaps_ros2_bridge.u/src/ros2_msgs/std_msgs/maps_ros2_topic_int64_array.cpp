...
void MAPSros2_topic_int64_array::Dynamic()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        m_outputName = Name();
        m_outputName += "_" + m_inOutName;
        NewOutput("output_int64_array", m_outputName.c_str());
...
bool MAPSros2_topic_int64_array::Birth()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        ...
        std::function<void(const std_msgs::msg::Int64MultiArray::SharedPtr msg)> fnc =
        std::bind(&MAPSros2_topic_int64_array::TopicCallback, this, std::placeholders::_1);
        m_sub = m_node->create_subscription<std_msgs::msg::Int64MultiArray>(m_topicName, 10, fnc);
    }
    else
    {
        m_pub = m_node->create_publisher<std_msgs::msg::Int64MultiArray>(m_topicName, 100);
    }

    return true;
}

void MAPSros2_topic_int64_array::Core()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
        return;
    ...
    {
        message.data[i] = ioeltin->Integer64(i);
    }
    ...
    message.layout.dim[0].stride=vectorsize_in*sizeof(int64_t);

    m_pub->publish(message);
}

void MAPSros2_topic_int64_array::Death()
{
    m_pub.reset();
    m_sub.reset();
}

void MAPSros2_topic_int64_array::TopicCallback(const std_msgs::msg::Int64MultiArray::SharedPtr msg)
{
    try 
    {
