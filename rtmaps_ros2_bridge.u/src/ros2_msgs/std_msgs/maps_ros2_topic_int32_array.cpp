...
void MAPSros2_topic_int32_array::Dynamic()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        m_outputName = Name();
        m_outputName += "_" + m_inOutName;
        NewOutput("output_int32_array", m_outputName.c_str());
    ...
    else
    {
        ...
        NewInput("input_int32", m_inputName.c_str());
    }
}

bool MAPSros2_topic_int32_array::Birth()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
    {
        ...
        std::function<void(const std_msgs::msg::Int32MultiArray::SharedPtr msg)> fnc =
        std::bind(&MAPSros2_topic_int32_array::TopicCallback, this, std::placeholders::_1);
        m_sub = m_node->create_subscription<std_msgs::msg::Int32MultiArray>(m_topicName, 10, fnc);
    }
    else
    {
        m_pub = m_node->create_publisher<std_msgs::msg::Int32MultiArray>(m_topicName, 100);
    }

    return true;
}

void MAPSros2_topic_int32_array::Core()
{
    if(m_endpointType == Endpoint_Type::Subscriber)
        return;
    ...
    {
        message.data[i] = ioeltin->Integer32(i);
    }
    ...
    message.layout.dim[0].stride=vectorsize_in*sizeof(int32_t);

    m_pub->publish(message);
}

void MAPSros2_topic_int32_array::Death()
{
    m_pub.reset();
    m_sub.reset();
}

void MAPSros2_topic_int32_array::TopicCallback(const std_msgs::msg::Int32MultiArray::SharedPtr msg)
{
    try 
    {
